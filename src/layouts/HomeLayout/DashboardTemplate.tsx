...
const DashboardTemplate = ({children} : any) => {
  ...
  useEffect(()=>{
    window.onclick = function(event : EventTarget | null | any){
      if(!(document.querySelector('.side-scroll-contain')?.contains(event.target) || 
  ...
  return (
    <div>
        ...
        <main className='flex flex-wrap h-[calc(100vh-80px)] lg:overflow-hidden '>
          <section className={`w-[280px] z-20 h-full side-scroll-contain hide-scrollbar top-0 lg:left-0 fixed lg:relative bg-white 
