  ...
  return Promise.reject(error);
   ...
   export function Get({method, url}) {
    ...
    useEffect(()=>{
       async function GET(){
        try{
            await server.get(BASE_URL + url,{timeout: setTimeout})
            .then((response)=>{
               setInfo({
                ...info,
                data:response,
...
export function Post({method,url, data}) {
   ...
   useEffect(()=>{
      async function POST(){
       try{
           await  server.post(BASE_URL + url,{
            ...data
           }, {timeout: setTimeout})
           .then((response)=>{
               setInfo({
   ...
   useEffect(()=>{
      async function PATCH(){
       try{
           await  server.patch(BASE_URL+url,{
            data
           }, {timeout: setTimeout})
           .then((response)=>{
               setInfo({
   ...
   export function Delete({method, url}) {
      ...
      useEffect(()=>{
         async function DELETE(){
          try{
              await server.delete(BASE_URL + url,{timeout: setTimeout})
              .then((response)=>{
                 setInfo({
                  ...info,
                  data:response,
