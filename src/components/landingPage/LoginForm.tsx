...
export default function LoginForm() {
  ...
  const submitUserInfo =(event: any)=>{
    ...
    setTimeout(() => {
      setisLoading(true)
      localStorage.setItem("lenszer-user-id", Math.floor(Math.random()*100) as unknown as string)
  ...
  return (
    <form onSubmit={submitUserInfo} className='w-full lg:w-[70%] flex justify-center p-2 lg:p-0'>
      <div className='w-full sm:w-[90%] md:w-[90%]'>
        <h1 className='text-[30px] text-lengsqr-500 font-extrabold header-form mb-2'>Welcome!</h1>
        <span className='text-[14px] text-lengsqr-500 header-label'>Enter datails to login</span>
        ...
        <div className='input-form'>
            <input placeholder='Password' required type={passwordVisible ? 'password' : 'text'}/>
