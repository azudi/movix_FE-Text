...
function Topnav(props: Props) {
  ...
  //Custom Hook
  const { data: ProfileData, isLoading} = Get({method:'GET', url:`/users/${localStorage.getItem("lenszer-user-id")}`})
   let dataInfo : any = {}
   if(ProfileData){
  ...
  return (
    <nav className='nav-container h-20'>
      ...
      <div className='w-6/12 lg:w-6/12 flex items-center justify-end lg:mr-5'>
        <span className='mx-3 underline text-[14px] text-lengsqr-500 hidden sm:inline-block cursor-pointer'>Docs</span>
          ...
          <div className='inline-flex mx-3 items-center'>
            <span className='w-10 h-10 mr-2'>
              <img src={dataInfo?.profile?.avatar || profileIcon} alt='' className='rounded-full overflow-hidden'></img>
            </span>
            <CDropdown>
              <CDropdownToggle color="transparent"> 
                       <span className='inline-flex item-center'>
                        <small className='hidden sm:inline'>{dataInfo?.profile?.firstName}</small>
                         <span className='sm:mt-1'><IoMdArrowDropdown/></span>
                         </span>
                         </CDropdownToggle>
              <CDropdownMenu style={{transform:'translate3d(-80px, 50px, 0px)'}}>
                <CDropdownItem>
                  <span className='inline-flex item-center w-full cursor-pointer' onClick={logout}>
                  <span className='mt-[5px]'><RiLogoutCircleLine/></span>
            ...
            <div onClick={toggleSideBar} className="cursor-pointer" id="nav-icon2" ref={navIcon}>
              <span></span>
              <span></span>
              <span></span>
              <span></span>
