...
function UserDetails(props: Props) {
    ...
    const { data: totalUser, isLoading,error, isError} = Get({method:'GET', url:`/users/${name}`})
    let dataInfo : any = {}
    if(totalUser){
      ...
      <DashboardTemplate>
        <section>
            <div className='w-full py-3 text-[19px] mt-4 headline'> 
             ...
             className='inline-flex items-center text-[14px] text-lengsqr-500 cursor-pointer'><LongArrowLeft width="25"/>&nbsp; Back to Users</span>
            </div>
            <div className='flex flex-wrap w-full items-align'>
               <div className='xs:w-6/12 w-full text-[18px] capitalize font-bold mt-2 text-lengsqr-500'> user details </div>
               <div className='xs:w-6/12 w-full lg:text-right mt-2'>
                  <button className='px-4 uppercase py-2 text-[11px] border-[2px] border-[#E4033B] text-[#E4033B] font-bold rounded-lg mr-3'>Blacklist User</button>
                  <button className='px-4 uppercase py-2 text-[11px] border-[2px] border-[#39CDCC] text-[#39CDCC] font-bold rounded-lg'>Activate User</button>
               </div>
                </div>
            <div>
